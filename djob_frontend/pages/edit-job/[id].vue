...
const { data: job } = await useFetch('http://127.0.0.1:8080/api/v1/jobs/' + route.params.id + '/')
...
async function submitForm() {
    ...
    if (title.value == '') { errors.value.push('Title field is missing.') }
    if (description.value == '') { errors.value.push('Description field is missing.') }
    if (position_salary.value == '') { errors.value.push('Position Salary field is missing.') }
    if (position_location.value == '') { errors.value.push('Position Location field is missing.') }
    if (company_name.value == '') { errors.value.push('Company Name field is missing.') }
    if (company_location.value == '') { errors.value.push('Company Location field is missing.') }
    if (company_email.value == '') { errors.value.push('Company Email field is missing.') }

    if (errors.value.length == 0) {
        await $fetch('http://127.0.0.1:8080/api/v1/jobs/' + route.params.id + '/edit/', {
            method: 'PUT',
            headers: {
