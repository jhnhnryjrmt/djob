...
async function deleteJob(id) {
  await $fetch('http://127.0.0.1:8080/api/v1/jobs/' + id + '/delete/', {
    method: 'DELETE',
    headers: {
...
<template>
  <div class="p-6 flex items-center justify-between bg-gray-100 rounded-xl">
    ...
    <div class="space-x-4">
      ...
      <NuxtLink v-bind:to="'/edit-job/' + job.id" class="py-4 px-6 bg-cyan-700 text-white rounded-xl" v-if="showButton">
        Edit</NuxtLink>
      <a @click="deleteJob(job.id)" class="py-4 px-6 bg-rose-700 text-white rounded-xl" v-if="showButton">
        Delete</a>
    </div>
  </div>
</template>
